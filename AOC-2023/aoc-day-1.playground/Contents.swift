...
func sumCalibrationValue(input: String) -> Int {
    let rawValues = input.split(separator: "\n")
    
    return rawValues.reduce(0) { partialResult, rawValue in
        
        let digits = rawValue.trimmingCharacters(in: .decimalDigits.inverted)
        
        if digits.count == 1, let digit = digits.first {
            let compoundDigit = "\(digit)\(digit)"
            return partialResult + (Int(compoundDigit) ?? 0)
        }

        if let first = digits.first, let last = digits.last {
            let compoundDigit = "\(first)\(last)"
            return partialResult + (Int(compoundDigit) ?? 0)
        }
...
func fixedSumCalibrationValue(input: String) -> Int {
    let rawValues = input.split(separator: "\n")
    ...
    return rawValues.reduce(0) { partialResult, rawValue in
        ...
        let fixedRawValue = rawValue
            ...
            .replacingOccurrences(of: "three", with: "th3ee")
            .replacingOccurrences(of: "four", with: "fo4ur")
            .replacingOccurrences(of: "five", with: "fi5ve")
            .replacingOccurrences(of: "six", with: "si6x")
            .replacingOccurrences(of: "seven", with: "sev7en")
            .replacingOccurrences(of: "eight", with: "ei8ght")
            .replacingOccurrences(of: "nine", with: "ni9ne")


        let digits = fixedRawValue.trimmingCharacters(in: .decimalDigits.inverted)
...
let part1 = sumCalibrationValue(input: inputValues)
let part2 = fixedSumCalibrationValue(input: inputValues)
