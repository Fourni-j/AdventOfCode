...
func computePossibleGames(input: String) -> Int {
    let games = input.split(separator: "\n")
    var validGameIds : Set<Int> = []
    
    gameloop: for game in games {
        guard let match = game.firstMatch(of: gameIdRegex),
              let gameId = Int(match.output.1) else { continue }
        
        let gameSets = game.split(separator: ";")
        
        for gameSet in gameSets {
            if let match = gameSet.firstMatch(of: redRegex),
            ...
            if let match = gameSet.firstMatch(of: greenRegex),
               let count = Int(match.output.1),
               count > greenLimit {
                // skip to next game
                continue gameloop
            ...
            if let match = gameSet.firstMatch(of: blueRegex),
               let count = Int(match.output.1),
               count > blueLimit {
                // skip to next game
                continue gameloop
        ...
        validGameIds.insert(gameId)
    }
    return validGameIds.reduce(0, +)
}


func computeCubePowerSet(input: String) -> Int {
    let games = input.split(separator: "\n")
    var powerSets: [Int] = []
    
    for game in games {

        let redMatches = game.matches(of: redRegex)
        let minValueRedMatch = redMatches.min { left, right in
            Int(left.output.1) ?? 0 > Int(right.output.1) ?? 0
        }
        let minRed = Int(minValueRedMatch?.output.1 ?? "1") ?? 1
        
        let greenMatches = game.matches(of: greenRegex)
        let minValueGreenMatch = greenMatches.min { left, right in
            Int(left.output.1) ?? 0 > Int(right.output.1) ?? 0
        }
        let minGreen = Int(minValueGreenMatch?.output.1 ?? "1") ?? 1

        let blueMatches = game.matches(of: blueRegex)
        let minValueBlueMatch = blueMatches.min { left, right in
            Int(left.output.1) ?? 0 > Int(right.output.1) ?? 0
        }
        let minBlue = Int(minValueBlueMatch?.output.1 ?? "1") ?? 1
        
        powerSets.append(minRed * minGreen * minBlue)
    }
    return powerSets.reduce(0, +)
}

computePossibleGames(input: inputValue)
computeCubePowerSet(input: inputValue)
